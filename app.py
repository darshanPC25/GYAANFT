...
prompt = ChatPromptTemplate.from_messages([
    ("system", """
    You are a helpful AI assistant that provides career guidance and answers questions about various career paths.
    Please provide accurate and helpful responses based on the context and collection data provided.
...
async def get_all_collections_data():
    try:
        collections = ["User", "Playlist", "Book", "CareerPath", "Question", "Other", "CareerAssessment"]
        all_data = {}
        ...
        logger.error(f"Error getting collections data: {str(e)}")
        raise
...
@app.get("/career_assessment/")
async def get_career_assessment():
    try:
        assessment_data = list(db.CareerAssessment.find({}, {'_id': 0}))
        if not assessment_data:
            raise HTTPException(status_code=404, detail="No assessment questions found")
        return {"questions": assessment_data}
    except Exception as e:
    ...
    try:
        ...
        if current_question_id == 1:
            ...
            try:
                ...
                # Store the category in the session or database for later use
